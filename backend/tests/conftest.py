...
from sqlalchemy import create_engine, String, Integer, Text, DateTime, Time, Enum as SQLEnum
...
class TestDatasets(TestBase):
    ...
    dataset_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    semester: Mapped[str] = mapped_column(String(10))
    upload_date: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    user_id: Mapped[str] = mapped_column(ForeignKey("users.user_id"))
    file_paths: Mapped[str] = mapped_column(Text, nullable=False)  # JSON as string for SQLite
...
class TestConflicts(TestBase):
    ...
    exam_assignment_ids: Mapped[str] = mapped_column(Text, nullable=False)  # JSON as string
...
class TestExamAssignments(TestBase):
    ...
    exam_assignment_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
...
class TestTimeSlots(TestBase):
    ...
    time_slot_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
