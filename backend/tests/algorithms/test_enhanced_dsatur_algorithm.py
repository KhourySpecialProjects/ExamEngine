...
class TestDSATURAlgorithm:
    ...
    def test_soft_penalty_calculation(
        ...
        penalty = graph._soft_tuple(
            student_sched, instr_sched, "1001", 3, 0
        )  # Thursday
        assert penalty[0] > 0  # Should have late penalty for Thursday

        penalty_early = graph._soft_tuple(
            student_sched, instr_sched, "1001", 1, 0
        )  # Tuesday
        assert penalty_early[0] == 0  # No penalty for Tuesday

    def test_scheduling_process(
        self, sample_census, sample_enrollment, sample_classrooms
    ):
        ...
        for _crn, (day, block) in assignment.items():
    ...
    def test_data_normalization(self):
        ...
        classrooms = pd.DataFrame(
            {"room_name": ["Room A", "Room B"], "capacity": [50, 40]}
        )
    ...
    def test_edge_cases(self):
        ...
        empty_enrollment = pd.DataFrame(
            columns=["student_id", "CRN", "instructor_name"]
        )
...
class TestAlgorithmIntegration:
    ...
    def test_full_workflow_with_real_data(self):
        ...
        try:
            ...
            graph.build_graph()
            print(
                f"Graph built: {graph.G.number_of_nodes()} nodes, {graph.G.number_of_edges()} edges"
            )
...
class TestAlgorithmPerformance:
    ...
    @pytest.mark.slow
    def test_algorithm_performance_large_dataset(
    ...
    def test_memory_usage_under_load(
        ...
        graphs = []
        for _i in range(5):  # Create 5 instances
            graph = DSATURExamGraph(
                large_census_data, large_enrollment_data, large_classroom_data
            )
            graph.build_graph()
...
class TestAdvancedConstraints:
    ...
    def test_instructor_scheduling_conflicts(self):
        ...
        for instructor, times in instructor_schedule.items():
            ...
            assert len(unique_times) == len(times), (
                f"Instructor {instructor} has conflicts: {times}"
            )
    ...
    def test_room_capacity_edge_cases(self):
        ...
        enrollment = pd.DataFrame(
            {
                "student_id": [
                    f"S{i:03d}" for i in range(90)
                ],  # 30 students per course
        ...
        for _, exam in schedule_df.iterrows():
            assert exam["Size"] <= exam["Capacity"], (
                f"Capacity violation: {exam['Size']} > {exam['Capacity']}"
            )
...
class TestAlgorithmOptimization:
    ...
    def test_coloring_optimality(self):
        ...
        for node in graph.G.nodes():
            ...
            for neighbor in graph.G.neighbors(node):
                ...
                assert node_color != neighbor_color, (
        ...
        assert num_colors <= len(census), (
            f"Too many colors used: {num_colors} > {len(census)}"
        )
    ...
    def test_room_utilization_optimization(self):
        ...
        assert avg_utilization < 100, (
            f"Room utilization over 100%: {avg_utilization:.1f}%"
        )
    ...
    def test_soft_constraint_prioritization(self):
        ...
        classrooms = pd.DataFrame(
            {"room_name": ["Room A", "Room B", "Room C"], "capacity": [60, 50, 40]}
        )
        ...
        if len(large_courses) > 0:
            ...
            assert early_ratio > 0.3, (
                f"Large courses not prioritized for early scheduling: {early_ratio:.2f}"
            )
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_impossible_scheduling_scenarios(self):
        ...
        classrooms = pd.DataFrame({"room_name": ["Room A"], "capacity": [100]})
        ...
        assert len(schedule_df) >= 0, (
            "Algorithm should not crash on impossible scenario"
        )

        # Should have some hard conflicts in impossible scenario
        _summary = graph.summary()
    ...
    def test_partial_scheduling_recovery(self):
        ...
        assert results[0] == len(census), (
            f"Not all courses scheduled: {results[0]} < {len(census)}"
        )
...
class TestAlgorithmComparison:
    ...
    def test_algorithm_vs_naive_approach(self):
        ...
        assert len(unique_slots) <= len(census), (
            f"DSATUR uses too many time slots: {len(unique_slots)} > {len(census)}"
        )
