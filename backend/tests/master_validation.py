...
def load_and_validate_data():
    ...
    try:
        ...
        print(
            f"   - Census num_students range: {census_df['num_students'].min()} - {census_df['num_students'].max()}"
        )
        print(
            f"   - Classroom capacity range: {classrooms_df['capacity'].min()} - {classrooms_df['capacity'].max()}"
        )
...
def run_scheduling_algorithm(
    census_df, enrollment_df, classrooms_df, algorithm_type="enhanced"
):
    ...
    try:
        ...
        census_processed["num_students"] = (
            pd.to_numeric(census_processed["num_students"], errors="coerce")
            .fillna(0)
            .astype(int)
        )
        ...
        student_col = (
            "Student_PIDM" if "Student_PIDM" in enrollment_df.columns else "student_id"
        )
        instructor_col = (
            "Instructor Name"
            if "Instructor Name" in enrollment_df.columns
        ...
        if student_col == "Student_PIDM":
            enrollment_processed = enrollment_processed.rename(
                columns={"Student_PIDM": "student_id"}
            )
        if instructor_col == "Instructor Name":
            enrollment_processed = enrollment_processed.rename(
                columns={"Instructor Name": "instructor_name"}
            )
        enrollment_processed["student_id"] = enrollment_processed["student_id"].astype(
            str
        )
        enrollment_processed["instructor_name"] = (
            enrollment_processed["instructor_name"].fillna("").astype(str)
        )

        # Process classroom data
        classrooms_processed = classrooms_df.copy()
        classrooms_processed["capacity"] = (
            pd.to_numeric(classrooms_processed["capacity"], errors="coerce")
            .fillna(0)
            .astype(int)
        )

        print("   - Data prepared for algorithm")
        ...
        graph.build_graph()
        print(
            f"   - Graph has {graph.G.number_of_nodes()} nodes and {graph.G.number_of_edges()} edges"
        )
        ...
        print(
            f"   - Hard student conflicts: {summary.get('hard_student_conflicts', 0)}"
        )
        print(
        ...
        print(
            f"   - Instructors with back-to-back: {summary.get('instructors_back_to_back', 0)}"
        )
        print(
            f"   - Large courses not early: {summary.get('large_courses_not_early', 0)}"
        )
...
def analyze_student_conflicts(schedule_df, enrollment_df, census_df):
    ...
    try:
        ...
        for _, exam in schedule_df.iterrows():
            ...
            student_col = (
                "Student_PIDM"
                if "Student_PIDM" in enrollment_df.columns
                else "student_id"
            )
            course_students = enrollment_df[enrollment_df["CRN"] == crn][
                student_col
            ].tolist()
        ...
        if not conflicts_df.empty:
            ...
            print("   SUCCESS: No student conflicts found!")
...
def analyze_capacity_violations(schedule_df):
    ...
    try:
        ...
        if not violations_df.empty and "Overflow" in violations_df.columns:
            ...
            print(
                f"   - Average overflow: {violations_df['Overflow'].mean():.1f} students"
            )
        else:
            print("   SUCCESS: No capacity violations found!")
...
def generate_comprehensive_report(
    ...
    try:
        ...
        hard_student_conflicts = (
            summary.get("hard_student_conflicts", 0) if summary else 0
        )
        ...
        instructors_back_to_back = (
            summary.get("instructors_back_to_back", 0) if summary else 0
        )
        large_courses_not_early = (
            summary.get("large_courses_not_early", 0) if summary else 0
        )
        ...
        schedule_df["Capacity_Utilization"] = (
            schedule_df["Size"] / schedule_df["Capacity"] * 100
        ).round(1)
        ...
        students_with_conflicts = (
            conflicts_df["Student_ID"].nunique()
            if conflicts_df is not None and not conflicts_df.empty
            else 0
        )
...
ROOM UTILIZATION (Top 10):"""
        ...
        avg_overflow = 0
        if (
            capacity_violations is not None
            and not capacity_violations.empty
            and "Overflow" in capacity_violations.columns
        ):
...
"""
        ...
        print(
            f"   SUCCESS: Comprehensive report saved to Data/{prefix}validation_report.txt"
        )

        # Generate and update ALGORITHM_VALIDATION_SUMMARY.md
        generate_algorithm_summary(
            schedule_df, detailed_schedule, conflicts_df, capacity_violations, summary
        )
        ...
        print(
            f"   - Exams placed: {placed_exams}/{total_exams} ({placed_exams / total_exams * 100:.1f}%)"
        )
...
def generate_algorithm_summary(
    ...
    try:
        ...
        # Size analysis
        schedule_df["Size"].mean()
        schedule_df["Size"].max()
        schedule_df["Size"].min()
        ...
        avg_utilization = schedule_df["Capacity_Utilization"].mean()
        schedule_df["Capacity_Utilization"].max()
        ...
        print(
            f"   - Capacity violations debug: {capacity_violations is not None}, {capacity_violations_count}"
        )
        if capacity_violations is not None and not capacity_violations.empty:
            print(
                f"   - Capacity violations columns: {capacity_violations.columns.tolist()}"
            )
        else:
            print("   - Capacity violations is None or empty")
...
"""
        ...
        print(
            "   SUCCESS: Algorithm validation summary updated: Data/ALGORITHM_VALIDATION_SUMMARY.md"
        )
...
def main(algorithm_type="enhanced"):
    ...
    try:
        ...
        graph, schedule_df, summary = run_scheduling_algorithm(
            census_df, enrollment_df, classrooms_df, algorithm_type
        )
        if graph is None:
        ...
        if success:
            ...
            print("   - Data/detailed_exam_schedule.csv")
            print("   - Data/student_conflicts_detailed.csv")
            print("   - Data/capacity_violations.csv")
            print("   - Data/validation_report.txt")
            print("   - Data/ALGORITHM_VALIDATION_SUMMARY.md (updated)")
            return True
        else:
            return False
