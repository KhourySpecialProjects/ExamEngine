...
class TestSchedulerBasic:
    ...
    def test_scheduler_initialization(
        ...
        scheduler = Scheduler(
            dataset=dataset, max_days=7, student_max_per_day=3, instructor_max_per_day=2
        )
    ...
    def test_complete_schedule(
        ...
        for _crn, (day, block) in result.assignments.items():
            assert 0 <= day < 7
            assert 0 <= block < 5

        # All rooms should exist in dataset
        for _crn, room_name in result.room_assignments.items():
            room_names = [r.name for r in dataset.rooms]
            assert room_name in room_names

    def test_no_hard_conflicts(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        ...
        for student_id, slots in student_schedule.items():
            assert len(slots) == len(set(slots)), (
                f"Student {student_id} has duplicate time slots"
            )
...
class TestSchedulerConstraints:
    ...
    def test_student_max_per_day(
        ...
        student_day_counts = {}
        for crn, (day, _block) in result.assignments.items():
            for student_id, student in dataset.students.items():
                if crn in student.enrolled_crns:
                    key = (student_id, day)
        ...
        assert violations < len(dataset.students), (
            "Too many student max_per_day violations"
        )
...
class TestSchedulerPrioritization:
    ...
    def test_prioritize_large_courses(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        ...
        course_sizes = {
            crn: dataset.get_enrollment_count(crn) for crn in dataset.courses
        }
...
class TestSchedulerEdgeCases:
    ...
    def test_empty_dataset(self):
        ...
        empty_dataset = SchedulingDataset(
            courses={}, students={}, rooms=[], students_by_crn={}, instructors_by_crn={}
        )
    ...
    def test_single_course(self, sample_classroom_data):
        ...
        single_enrollment = pd.DataFrame({"Student_PIDM": ["S001"], "CRN": ["1001"]})
    ...
    def test_no_conflicts(self):
        ...
        enrollments = pd.DataFrame(
            {"Student_PIDM": ["S001", "S002", "S003"], "CRN": ["1001", "1002", "1003"]}
        )

        rooms = pd.DataFrame(
            {"room_name": ["Room A", "Room B", "Room C"], "capacity": [50, 50, 50]}
        )
...
class TestSchedulerPerformance:
    ...
    def test_large_dataset(
        self, large_census_data, large_enrollment_data, large_classroom_data
    ):
    ...
    def test_many_conflicts(self):
        ...
        courses = pd.DataFrame(
            {
                ...
                "Instructor Name": [
                    f"Dr. Instructor{i % 10}" for i in range(num_courses)
                ],
        ...
        for student_id in range(num_students):
            ...
            for crn_idx in student_courses:
                enrollments.append(
                    {"Student_PIDM": f"S{student_id:04d}", "CRN": f"{1000 + crn_idx}"}
                )

        enrollment_df = pd.DataFrame(enrollments)

        rooms = pd.DataFrame(
            {"room_name": [f"Room {i}" for i in range(30)], "capacity": [50] * 30}
        )
...
class TestSchedulerMergedCourses:
    ...
    def test_merged_courses_same_time_slot(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        ...
        large_room = pd.DataFrame({"room_name": ["Large Room"], "capacity": [60]})
        sample_classroom_data = pd.concat(
            [sample_classroom_data, large_room], ignore_index=True
        )
        ...
        merges = {"merge_1": ["1001", "1002"]}

        scheduler = Scheduler(dataset=dataset, max_days=7, merges=merges)
    ...
    def test_merged_courses_room_capacity(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        ...
        room_capacity = next(
            (r.capacity for r in dataset.rooms if r.name == room_name), None
        )
    ...
    def test_multiple_merge_groups(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        ...
        sample_classroom_data = sample_classroom_data.copy()
        large_room = pd.DataFrame({"room_name": ["Large Room"], "capacity": [65]})
        sample_classroom_data = pd.concat(
            [sample_classroom_data, large_room], ignore_index=True
        )
        ...
        merges = {"merge_1": ["1001", "1002"], "merge_2": ["1003", "1004"]}
    ...
    def test_merged_courses_same_color(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
    ...
    def test_merged_courses_conflict_detection(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
    ...
    def test_empty_merges(
        ...
        scheduler = Scheduler(dataset=dataset, max_days=7, merges={})
    ...
    def test_merge_with_nonexistent_crn(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
...
class TestSchedulerRoomBlockouts:
    """Tests for room blockout constraint enforcement."""

    def _make_dataset(self, courses_data, enrollments_data, rooms_data, blockouts=None):
        """Build a SchedulingDataset directly from domain objects."""
        ...
        courses = {
            row["crn"]: Course(
                ...
                enrollment_count=row["enrollment"],
        ...
        rooms = [Room(name=r["name"], capacity=r["capacity"]) for r in rooms_data]

        student_courses = defaultdict(set)
        for sid, crn in enrollments_data:
            student_courses[sid].add(crn)

        students = {
            sid: Student(student_id=sid, enrolled_crns=frozenset(crns))
            for sid, crns in student_courses.items()
        }
        students_by_crn = defaultdict(set)
        for sid, crns in student_courses.items():
            for crn in crns:
        ...
        return SchedulingDataset(
            ...
            rooms=rooms,
            students_by_crn={
                crn: frozenset(sids) for crn, sids in students_by_crn.items()
            },
            instructors_by_crn={},
            room_blockouts=blockouts or {},
        )

    def test_blocked_room_not_assigned_at_blocked_slot(self):
        """A room that is blocked at a slot must not receive any exam at that slot."""
        dataset = self._make_dataset(
            courses_data=[
                {"crn": "C1", "course_code": "CS 101", "enrollment": 10},
                {"crn": "C2", "course_code": "CS 102", "enrollment": 5},
            ],
            enrollments_data=[("S1", "C1"), ("S2", "C2")],
            rooms_data=[
        ...
        for crn, room in result.room_assignments.items():
            day, block = result.assignments[crn]
            # Room A is blocked for every slot — should never be assigned
            assert room != "Room A", (
                f"Course {crn} was assigned to fully-blocked Room A at ({day}, {block})"
            )

    def test_blocked_slot_uses_other_room(self):
        """When one room is blocked, the scheduler uses another available room."""
        # Block Room A on day 0, block 0 only
        blockouts = {"Room A": frozenset([(0, 0)])}

        dataset = self._make_dataset(
            courses_data=[{"crn": "C1", "course_code": "CS 101", "enrollment": 10}],
            enrollments_data=[("S1", "C1")],
            rooms_data=[
                {"name": "Room A", "capacity": 50},
                {"name": "Room B", "capacity": 50},
            ],
        ...
        day, block = result.assignments["C1"]
        room = result.room_assignments.get("C1")

        if day == 0 and block == 0:
            assert room != "Room A", "Room A is blocked at (0,0) but was still assigned"

    def test_no_blockouts_preserves_normal_behavior(
        self, sample_census_data, sample_enrollment_data, sample_classroom_data
    ):
        """With no blockouts, behavior is identical to a regular run."""
        dataset_no_blockouts = DatasetFactory.from_dataframes_to_scheduling_dataset(
            sample_census_data, sample_enrollment_data, sample_classroom_data
        )
        dataset_empty_blockouts = DatasetFactory.from_dataframes_to_scheduling_dataset(
            sample_census_data,
        ...
        scheduler1 = Scheduler(dataset=dataset_no_blockouts, max_days=7)
        result1 = scheduler1.schedule()

        scheduler2 = Scheduler(dataset=dataset_empty_blockouts, max_days=7)
    ...
    def test_all_rooms_blocked_at_slot_course_gets_no_room(self):
        ...
        dataset = self._make_dataset(
            courses_data=[{"crn": "C1", "course_code": "CS 101", "enrollment": 10}],
            enrollments_data=[("S1", "C1")],
            rooms_data=[{"name": "Room A", "capacity": 50}],
            blockouts=blockouts,
        )
    ...
    def test_blockout_from_dataframe_end_to_end(self):
        ...
        # Block Room A on day 0 for ALL 5 blocks
        ...
        dataset = DatasetFactory.from_dataframes_to_scheduling_dataset(
            courses_df, enrollments_df, rooms_df, blockouts_df=blockouts_df
        )

        assert "Room A" in dataset.room_blockouts
        assert len(dataset.room_blockouts["Room A"]) == 5
        ...
        for crn in result.assignments:
            ...
            if day == 0:
                assert room != "Room A", (
                    f"Course {crn} placed in blocked Room A on day 0 block {block}"
                )

    def test_multiple_rooms_partially_blocked(self):
        """Partial blockouts allow other rooms to be used at blocked slots."""
        # Room A blocked on day 0; Room B available everywhere
        blockouts = {"Room A": frozenset([(0, 0), (0, 1), (0, 2), (0, 3), (0, 4)])}

        dataset = self._make_dataset(
            courses_data=[
                {"crn": "C1", "course_code": "CS 101", "enrollment": 10},
                {"crn": "C2", "course_code": "CS 102", "enrollment": 10},
            ],
            enrollments_data=[("S1", "C1"), ("S2", "C2")],
            rooms_data=[
                {"name": "Room A", "capacity": 50},
