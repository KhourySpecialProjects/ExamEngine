...
from src.services.validation import validate_csv_schema, get_file_statistics
...
async def upload_dataset(
    ...
    for file_type, upload_file in uploaded_files.items():
        try:
            ...
            if missing_cols:
                validation_errors[file_type] = f"Missing columns: {', '.join(missing_cols)}"
                continue
            
            stats = get_file_statistics(df, file_type, len(content), upload_file.filename)
...
async def list_datasets(
    ...
    try:
        ...
        ).order_by(Datasets.upload_date.desc()).all()
