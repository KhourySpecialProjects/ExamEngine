...
class TestUsersModel:
    ...
    def test_create_user(self, test_db):
        ...
        user = TestUsers(
            name="John Doe", email="john@example.com", password_hash="hashed_password"
        )
        test_db.add(user)
        test_db.commit()
    ...
    def test_user_unique_email(self, test_db):
        ...
        user1 = TestUsers(
            name="User 1", email="same@example.com", password_hash="hash1"
        )
        user2 = TestUsers(
            name="User 2", email="same@example.com", password_hash="hash2"
        )
...
class TestRoomsModel:
    ...
    def test_room_capacity_required(self, test_db, sample_dataset):
        ...
        room = TestRooms(location="Test Room", dataset_id=sample_dataset.dataset_id)
        test_db.add(room)
        with pytest.raises(IntegrityError):  # Should raise validation error
            test_db.commit()
...
class TestTimeSlotsModel:
    ...
    def test_all_day_enum_values(self, test_db, sample_dataset):
        ...
        days = [
            TestDayEnum.Monday,
            TestDayEnum.Tuesday,
            TestDayEnum.Wednesday,
            TestDayEnum.Thursday,
            TestDayEnum.Friday,
        ]
...
class TestSchedulesModel:
    ...
    def test_create_schedule(self, test_db, sample_run):
        ...
        schedule = TestSchedules(
            schedule_name="Final Exam Schedule 2024", run_id=sample_run.run_id
        )
    ...
    def test_schedule_name_unique(self, test_db, sample_run):
        ...
        schedule1 = TestSchedules(
            schedule_name="Unique Schedule", run_id=sample_run.run_id
        )
...
class TestExamAssignmentsModel:
    ...
    def test_create_exam_assignment(
        self, test_db, sample_course, sample_time_slot, sample_room, sample_schedule
    ):
